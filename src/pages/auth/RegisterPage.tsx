...
export function RegisterPage({ onNavigate }: RegisterPageProps) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setSubmitting(true)
    try {
      await register({ name, email, password, wantToHost: wantHost })
      const target = wantHost ? '/host/listings' : '/me/bookings'
  ...
  return (
    <div className="min-h-screen bg-dusty-mauve-50">
      ...
      <div className="container flex justify-center py-16">
        <div className="w-full max-w-xl rounded-3xl border border-white/60 bg-white/80 p-8 shadow-soft">
          <p className="text-xs uppercase tracking-widest text-dry-sage-400">Регистрация</p>
          <h1 className="mt-1 text-3xl font-semibold text-dusty-mauve-900">Создайте аккаунт Rentme</h1>
          <p className="mt-2 text-sm text-dusty-mauve-500">
            Один профиль — для путешествий и сдачи жилья. Позже можно добавить новые роли.
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-5">
            ...
            <label className="flex items-center gap-3 text-sm text-dusty-mauve-700">
              <input
                type="checkbox"
                checked={wantHost}
                onChange={(event) => setWantHost(event.target.checked)}
                className="h-5 w-5 rounded border-dusty-mauve-300 text-dry-sage-600 focus:ring-dry-sage-500"
              />
              Хочу сразу размещать жильё как хост
            </label>
            {error && (
          ...
          <p className="mt-6 text-center text-sm text-dusty-mauve-500">
            ...
            <button
              ...
              onClick={() => withViewTransition(() => onNavigate('/login'))}
            >
              Войти
            </button>
          </p>
